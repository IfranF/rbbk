...
def cleanup_test_results(destination='/dbfs/user/hive/warehouse/tests/foldertests/SCD2_incremental_table'): # Cleanup the test results before starting historied delta table
    
    try:
        print("Cleaning up before testing")
        #destination = '/dbfs/user/hive/warehouse/tests/foldertests/SCD2_incremental_table'
        print(destination)
        dbutils.fs.rm(destination, True)
    except Exception as e:
        print("Error while deleting the folders", e)

def test_generic_scd2_incremental_day1_positive(location="test_scenario_1"):
    ...
    destination_path = f'{base_path}/tests/foldertests/SCD2_incremental_table/{location}'
...
def test_generic_scd2_incremental_day2_positive(location='test_scenario_2'):
    ...
    try:
        ...
        test_generic_scd2_incremental_day1_positive(location)
...
def test_generic_scd2_incremental_day3_merge_schema():
    ...
    # Get the destination path
    base_path = "/dbfs/user/hive/warehouse" #Path(os.getcwd()).parent.parent
    destination_path = f'{base_path}/tests/foldertests/SCD2_incremental_table/test_scenario_3'
    print("destination_path: ", destination_path)
    ...
    try:
        ...
        # create test data with first day SCD2 run
        test_generic_scd2_incremental_day2_positive("test_scenario_3")
...
def test_generic_scd2_incremental_day2_merge_schema_false():
    ...
    # Get the destination path
    base_path = "/dbfs/user/hive/warehouse" #Path(os.getcwd()).parent.parent
    destination_path = f'{base_path}/tests/foldertests/SCD2_incremental_table/test_scenario_4'
    print("destination_path: ", destination_path)
    ...
    # create test data with first day SCD2 run
    test_generic_scd2_incremental_day1_positive("test_scenario_4")
    print("test data for day 1 created successfully!")

    # Call the function
    # generic_SCD2_incremental(source_key_nonkey_df,destination_path,'start_date_id', 'START_DATE', primary_key_hash= 'EDL_KEY_HASH',non_primary_key_hash='EDL_NONKEY_HASH',edl_valid_from_date='EDL_VALID_FROM_DTS',edl_valid_to_date="EDL_VALID_TO_DTS",edl_active_flag="EDL_ACTIVE_FLG",edl_deleted_flg="EDL_DELETED_FLG",edl_load_date='EDL_LOAD_DTS',merge_schema=False)

    # Assert the result
    with pytest.raises(Exception, match="Error occured during upsert: SCD2 incremental failed"):
        generic_scd2(source_key_nonkey_df, 
                                 destination_path, 
                                 'start_date_id', 
                                 'START_DATE',
...
if __name__ == "__main__":
    ...
    test_generic_scd2_incremental_day1_positive()
    test_generic_scd2_incremental_day2_positive()
    test_generic_scd2_incremental_day3_merge_schema()
    test_generic_scd2_incremental_day2_merge_schema_false()
    cleanup_test_results()
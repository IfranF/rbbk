...
def cleanup_test_results(): # Cleanup the test results before starting historized delta table
    
    try:
        print("Cleaning up before testing")
        destination = '/dbfs/user/hive/warehouse/tests/foldertests/SCD1_table'
        print(destination)
        dbutils.fs.rm(destination, True)
    except Exception as e:
        print("Error while deleting the folders", e)

def test_generic_scd1_day1_positive(location="test_scenario_1"):
    ...
    expected_schema = StructType([
        ...
        StructField("EDL_UPDATE_DTS", TimestampType(), True),
    ...
    destination_path = f'{base_path}/tests/foldertests/SCD1_table/{location}'
    ...
    try:
        ...
        generic_scd1(source_key_nonkey_df,
                        ...
                        edl_update_datetime='EDL_UPDATE_DTS',
    ...
    result_df=result_df.withColumn("EDL_LOAD_DTS", date_format("EDL_LOAD_DTS",'yyyy-MM-dd hh'))\
        .withColumn("EDL_UPDATE_DTS", date_format("EDL_UPDATE_DTS",'yyyy-MM-dd hh'))
    expected_df=expected_df.withColumn("EDL_LOAD_DTS", date_format("EDL_LOAD_DTS",'yyyy-MM-dd hh'))\
        .withColumn("EDL_UPDATE_DTS", date_format("EDL_UPDATE_DTS",'yyyy-MM-dd hh'))
    
    result_df_sorted=result_df.select(columns_to_compare).sort(*result_df.columns).orderBy(['CUSTOMER_ID', 'EDL_UPDATE_DTS'])
    expected_df_sorted=expected_df.select(columns_to_compare).sort(*expected_df.columns).orderBy(['CUSTOMER_ID', 'EDL_UPDATE_DTS'])
...
def test_generic_scd1_day2_positive():
    ...
    # Get the destination path
    base_path = "/dbfs/user/hive/warehouse" #Path(os.getcwd()).parent.parent
    destination_path = f'{base_path}/tests/foldertests/SCD1_table/test_scenario_2'
    print("destination_path: ", destination_path)
    ...
    try:
        ...
        test_generic_scd1_day1_positive("test_scenario_2")
...
def test_generic_scd1_day2_merge_schema():
    ...
    # Get the destination path
    base_path = "/dbfs/user/hive/warehouse" #Path(os.getcwd()).parent.parent
    destination_path = f'{base_path}/tests/foldertests/SCD1_table/test_scenario_3'
    print("destination_path: ", destination_path)
    ...
    try:
        ...
        # create test data with first day SCD2 run
        test_generic_scd1_day1_positive("test_scenario_3")
        print("test data for day 1 created successfully!")
...
def test_generic_scd1_day2_merge_schema_false():
    ...
    # Get the destination path
    base_path = "/dbfs/user/hive/warehouse" #Path(os.getcwd()).parent.parent
    destination_path = f'{base_path}/tests/foldertests/SCD1_table/test_scenario_4'
    print("destination_path: ", destination_path)
    ...
    # create test data with first day SCD2 run
    test_generic_scd1_day1_positive("test_scenario_4")
    print("test data for day 1 created successfully!")

    # Assert the result
    with pytest.raises(Exception, match="Error occured during upsert: SCD1 failed"):
        generic_scd1(source_key_nonkey_df, 
                        destination_path, 
                        'start_date_id', 
                        'START_DATE', 
...
if __name__ == "__main__":
    ...
    test_generic_scd1_day1_positive()
    test_generic_scd1_day2_positive()
    test_generic_scd1_day2_merge_schema()
    test_generic_scd1_day2_merge_schema_false()
    cleanup_test_results()